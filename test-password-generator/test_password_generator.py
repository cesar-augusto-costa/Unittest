# Testes com Python - Gerador de senhas com Unittest e TDD
...
from password_generator import (make_one_lowercase_letter, make_one_number,
                                make_one_special_char,
                                make_one_uppercase_letter, make_password)
...
class TestPasswordGenerator(unittest.TestCase):
    ...
    def test_make_password_length(self):
        ...
        with self.assertRaises(AssertionError):
            make_password(lengthy=1)

        with self.assertRaises(AssertionError):
            make_password(lengthy='a')  # type: ignore

        for i in range(4, 100):
            with self.subTest(i=i):
                self.assertTrue(len(make_password(lengthy=i)) == i)
    ...
    def test_make_password_all_params_false(self):
        with self.assertRaises(AssertionError):
            make_password(numbers=False, upper=False,
                          lengthy=False, chars=False)

    def test_make_password_no_upper(self):
        regex = re.compile(r'^[^A-Z]+$')
        for i in range(4, 100):
            with self.subTest(i=i):
                self.assertRegex(make_password(lengthy=i, upper=False), regex)
    ...
    def test_make_password_no_numbers(self):
        ...
        for i in range(4, 100):
            with self.subTest(i=i):
                self.assertRegex(
                    make_password(lengthy=i, numbers=False), regex
                )
    ...
    def test_make_password_no_chars(self):
        ...
        for i in range(4, 100):
            with self.subTest(i=i):
                self.assertRegex(make_password(lengthy=i, chars=False), regex)

    def test_make_password_sequence_not_allowed(self):
        regex = re.compile(r'^(?:[ -\/:-@\[-`{-~][a-z][A-Z][0-9])+$')
        for i in range(4, 100):
            with self.subTest(i=i):
                self.assertNotRegex(make_password(lengthy=i), regex)
